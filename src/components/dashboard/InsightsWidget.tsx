...
export const InsightsWidget = ({ workspaceId }: InsightsWidgetProps) => {
  ...
  const getSeverityClass = (severity: string) => {
    switch (severity) {
      case 'critical': return 'border border-red-200 bg-red-50/50';
      case 'warning': return 'border border-amber-200 bg-amber-50/50';
      default: return 'border border-slate-200 bg-slate-50/30';
    }
  };
      ...
      <CardContent className="space-y-2">
        {insights.length === 0 ? (
          ...
          insights.map(insight => (
            <div
              key={insight.id}
              className={`relative p-3 rounded-lg ${getSeverityClass(insight.severity)} transition-all hover:shadow-sm`}
            >
              <button
