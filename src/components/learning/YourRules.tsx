...
const classificationColor = (cls: string): string => {
  const map: Record<string, string> = {
    ...
    notification: 'bg-background-alt text-foreground/80 border-border',
  ...
  return map[cls] || 'bg-background-alt text-foreground/80 border-border';
};
    ...
    <div className="bg-white rounded-2xl border border-border shadow-sm p-6 flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-foreground">Your rules</h2>
        </div>
        <span className="text-xs text-muted-foreground/70">{rules.length} active</span>
      </div>

      {rules.length === 0 && !showAddForm ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          <p className="text-sm text-muted-foreground">No rules yet. Review emails in the Teach queue to start teaching BizzyBee.</p>
        </div>
      ) : (
              ...
              {editingId === rule.id ? (
                <div className="space-y-2">
                  <p className="text-sm text-muted-foreground">
                    Emails from <span className="font-medium text-foreground">{rule.sender_pattern}</span>
                  </p>
                ...
                <div className="flex items-center justify-between">
                  <p className="text-sm text-foreground/80">Remove this rule?</p>
      ...
      {showAddForm ? (
        ...
        <Button
          variant="outline"
          size="sm"
          className="mt-3 w-full text-foreground/80 hover:bg-background-alt"
